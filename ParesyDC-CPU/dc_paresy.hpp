...
midSplit(const vector<string>& vec) {
    ...
    const vector<string>& pos, const vector<string>& neg)
{
    ...
    if (r21RejectsTheWholeN2) {
        ...
        bool r22CoversRemainingNeg = none_of(r22Filter.begin(), r22Filter.end(), [](bool val) { return val; });
        ...
        if (matchesAll(posMinusP2MinusLeft, right)) {
            return right;
        }
    }
